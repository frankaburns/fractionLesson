...
/**
 * The ProblemGenerator class builds a List of BasicFractionalMath objects.  The generated list is defined by
...
class FractionProblemGenerator {
    ...
    //
    // A list of BasicFractionalMath objects, the main lesson data.
    List<BasicFractionalMath> problemSet = new ArrayList<>();
     ...
     *  - numProblems - 10
     */
    public FractionProblemGenerator() {
        endRow         = 10;
        endCol         = 10;
        startRow       = 0;
        startCol       = 0;
    ...
    /**
     * Get the next BasicFractionalMath object i.e. problem
     * @return get he next problem in the list
     */
    public BasicFractionalMath getProblem () {
    ...
    /**
     * Get a problem (BasicFractionalMath object) by index
     * @param index - the index in the ProblemSet List
     * @return - the BasicFractionalMath object referenced by index.
     */
    public BasicFractionalMath getProblem (int index) {
    ...
    private void shuffleArray () {
      ...
      for( int prob = 0; prob < problemSet.size(); prob++ ){
          ...
          BasicFractionalMath tmp = problemSet.get(newPosition);
    ...
    public void buildProblemSet () {
        ...
        if (lessonFunction == BasicFractionalMath.ADD || lessonFunction == BasicFractionalMath.MUL) {
        ...
        } else if (lessonFunction == BasicFractionalMath.DIV) {
            for( int i = startRow, iPlusOne = (startRow == 0) ? 1 : startRow; i < endRow; i++, iPlusOne++ ) {
                for( int j = startCol, jPlusOne = 2; jPlusOne < endCol; j++, jPlusOne++ ){
                    Fraction frac1 = new Fraction(iPlusOne, jPlusOne);
                    for( int k = startCol, kPlusOne = 2; kPlusOne < endCol; k++, kPlusOne++ ) {
